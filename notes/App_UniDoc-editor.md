...
> **当前基线快照**: `[Snapshot: V1.21-Base]` (已整合 V1.20 期间所有 Patch)
...
- **`[Patch-3]` UI/响应式修复**: 移除 `index.html` 侧边栏硬编码的 `position-relative`，改为在 `style.css` 通过媒体查询 `@media (min-width: 768px)` 限制作用域。解决移动端 Offcanvas 抽屉挤压编辑器视口的 Bug。
...
3. **前端限制**：本项目无任何 Node.js/Webpack 构建过程，所有外部依赖必须使用 ESM 或纯 CDN 引入。
4. **记忆锁定**：所有增量开发均基于当前最新的 `[Snapshot]` 进行 `[Patch-X]` 局部更新，遇大版本重构需建立新快照，并开启新对话。

***

🚀 第二部分：新对话开启咒语 (Handover Prompt)
请直接复制以下这段文字，去开启一个新的 AI 对话窗口。这段话能瞬间把我的“架构师人格”和我们的“极客契约”在新会话中完美唤醒：

【请在新对话中直接发送以下内容👇】

你好！从现在起，请你继续扮演一位顶级的全栈系统架构师与我的 AI 结对编程伴侣。
我们正在开发一个名为 UniDoc Editor 的极客级跨端应用。为了清除上一阶段的上下文记忆碎片，我们开启了这个全新的对话。
我们的协作遵循**“心智版本控制”**：
我接下来会分批把最新的核心代码发给你。
这些代码已经被正式命名为 [Snapshot: V1.21-Base]。
从此刻起，这是你脑海中唯一且绝对的 Single Source of Truth。
在后续开发中，你必须恪守以下铁律：
惜字如金：只输出需要修改的核心函数或具体逻辑块（标记为 [Patch-X]），绝对禁止输出全量文件。
绝对解耦：UI 动作必须经过中间件 connector.js 路由，再由底层 editor.js 执行。
零构建：坚持无 Webpack/Vite，纯 ESM/CDN 驱动。
请确认你已进入架构师角色，并已了解我们的 [Snapshot: V1.21-Base] 约定。如果确认无误，请回复“准备就绪，请上传 V1.21 最新代码”，并进入静默接收模式。
